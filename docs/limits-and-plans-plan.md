...
**Status: implemented and verified against a live dev store 2026-09-18.** Written against commit `66feb4f`. Every recommendation in
§15 was accepted; the plan text already assumes them. Deviations are recorded in §16.
...
| ----- | -------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2     | First-sync filter `status:open fulfillment_status:unshipped`.                                                                    | `status:open -fulfillment_status:fulfilled`.                                                                                                                                                     | §4 said to check the terms against `refs/shopify-docs` and deviate if they disagree. They do: the Admin REST reference defines `unshipped` as `fulfillment_status` **null**, and the GraphQL order-search reference lists `partial` as a separate value — so `unshipped` would have dropped partially fulfilled orders, which are open work. `NOT`/`-` is documented in `refs/shopify-docs/docs/api/usage/search-syntax.md`. |
| 5     | `sweepExpiredAuth` from the `sendMagicLink` callback "on both the demo and real branches".                                       | Once, before the branch.                                                                                                                                                                         | The two branches differ in how the link is delivered, not in what they store; one call covers both and cannot drift.                                                                                                                                                                                                                                                                                                         |
| 8     | `addMember` inside a `sqlPrimary.withTransaction`.                                                                               | Count-then-insert, no transaction, race accepted and documented on the method.                                                                                                                   | D1 has no interactive transactions — the driver rejects `withTransaction`, which is why `setMemberTeams` already uses `db.batch()` — and a batch cannot branch on a count. The plan anticipated this and authorised the deviation.                                                                                                                                                                                           |
| 9     | `setRun`'s `WorkflowRunLimitError` mapped "in the order page's server fn the same way other repository errors are mapped there". | Mapped to a new `Domain.AttachResult` variant `RunLimit { limit }`, with copy in `attachResultMessage`.                                                                                          | Attach is not a server fn: it is `ShopAgent.attachWorkflow`, a `@callable()` whose expected failures are already values in `AttachResult`. A thrown error there would reach the socket seam as a 500 instead of a sentence.                                                                                                                                                                                                  |
| 9     | Clear `liveRunsLimitedAt` in `recomputeStatus` and `cancelRun`/`cancelPending`, unconditionally.                                 | Same three call sites, but through `releaseLiveRunLimit`, which reads the flag first and only counts (and clears) when it is set.                                                                | Clearing unconditionally would drop the banner while the shop was still at the ceiling — `recomputeStatus` also fires on `pending → active`, which frees nothing. Reading one row first keeps the never-limited case (all of them, in practice) free of the count.                                                                                                                                                           |
| 9, 11 | Test seam: "make `insertRun` read the limit from a parameter" or "expose the constant through the repository layer".             | The tests lower `Domain.ShopLimits.maxLiveRuns` / `storageSoftLimitBytes` for their duration (`withMaxLiveRuns` in `workflow-run-repository.test.ts`, inline in `orders-sync-workflow.test.ts`). | Both seams would have put a parameter in a production signature for one caller that is a test. The constants are plain objects behind a `readonly` type, and the override is scoped and restored.                                                                                                                                                                                                                            |
| 11    | `repository.failSync({ message })` for the storage refusal.                                                                      | New `OrderRepository.setSyncError({ error })`.                                                                                                                                                   | `failSync` releases a reservation identified by its `startedAt`, and the guard runs _before_ `reserveSync`, so its `where id = 1 and startedAt = ?` would have matched nothing and written no error.                                                                                                                                                                                                                         |
| 11    | `rowsRead` instrumentation "optional".                                                                                           | Not done.                                                                                                                                                                                        | The Effect SQL client does not surface the underlying `SqlStorageCursor`, which the plan named as the condition. `databaseSize` is logged at both ends of every import, as required.                                                                                                                                                                                                                                         |
| 12    | `/app` and the orders index each render the banners.                                                                             | Both do, from `src/components/QuotaBanners.tsx`; on the orders index `usage` and the plan ceiling come from the loader and deliberately do **not** refresh with the socket.                      | A quota is a monthly fact and a plan an even slower one; refetching either on every order push would be a Durable Object read per webhook for a number that moves on a scale of days. Documented on `Domain.OrdersIndexLoaderData`.                                                                                                                                                                                          |
| 12    | Admin page shows usage.                                                                                                          | Shown, and the loader calls `ShopAgentClient.getUsage` — the first call this page makes to the object.                                                                                           | Only in the `Found` branch, after the missing-`ShopSession` early return, so viewing an arbitrary shop path still cannot materialise an empty Durable Object. The page's JSDoc says so.                                                                                                                                                                                                                                      |
| 7     | One detail-page banner for `lineItemsTruncated`.                                                                                 | Two banners, mutually exclusive: the existing `!lineItemsComplete` one, and a new one for `lineItemsTruncated && lineItemsComplete`.                                                             | Setting `lineItemsTruncated = !lineItemsComplete` on the single-order path (as §8 asks) makes the flag true in both cases, and one message cannot be accurate for both: the single-order path pages at `ORDER_SYNC_LINE_ITEMS` (100) and merges, the bulk path drops at `maxLineItemsPerOrder` (250).                                                                                                                        |
| 12    | Admin page shows the plan's `maxMembers`.                                                                                        | Shows `memberCount` of `maxMembers`, from a `Repository.countMembers` call added to the loader; `databaseSize` renders with one decimal place.                                                   | Found in the live app: beside "Orders this month: 15 of 250", a bare "Members: 3" reads as a count rather than a ceiling, and 425,984 bytes rendered as "0 MB".                                                                                                                                                                                                                                                              |
| —     | (nothing; pre-existing copy)                                                                                                     | Dropped "Billing is disabled, so every shop is granted the widest tier." from the `/app` plan card.                                                                                              | `BILLING_ENABLED` is now `"true"` locally, which made the sentence false on the page. The flag is per environment, so no merchant-facing sentence should assert its state.                                                                                                                                                                                                                                                   |
| 8     | `countMembers` / `countTeams` as repository-local helpers.                                                                       | Both also exposed on the `Repository` service interface.                                                                                                                                         | `countMembers` is read by two loaders (`/app`, the admin shop page) for the "n of m" lines, and `countTeams` by `repository.test.ts` to assert the cap from outside. Neither is reachable otherwise.                                                                                                                                                                                                                         |
| 7     | `OrdersStreamCounts` gains `ordersInserted`.                                                                                     | Gains `ordersInserted` **and** `ordersTruncated`; `onOrdersStream` now returns the named `OrdersStreamCounts` type rather than an inline shape.                                                  | A truncated order is logged per order at warning level, but the per-import total is what tells an operator whether the cap fired at all, and it costs one integer in a fold that was already counting three.                                                                                                                                                                                                                 |
| —     | `BILLING_ENABLED` per environment is the project owner's call.                                                                   | `"true"` in all three environments, on purpose.                                                                                                                                                  | Set by the project owner so it is not forgotten; staging and production are not deployed yet, and their `SHOPIFY_PARTNER_APP_ID` must be filled and their plans created before they are.                                                                                                                                                                                                                                     |
| 9     | (cosmetic)                                                                                                                       | Review 2026-09-18: `cancelPending` calls `releaseLiveRunLimit` through `Effect.tap(() => …)` like the other two sites.                                                                           | Consistency.                                                                                                                                                                                                                                                                                                                                                                                                                 |
| —     | (pre-existing) JSDoc on `merchantCompleteStep` referenced a deleted `docs/` file.                                                | Review 2026-09-18: reasoning inlined.                                                                                                                                                            | Project rule: JSDoc carries its reasoning and never points at `docs/`.                                                                                                                                                                                                                                                                                                                                                       |
| 12    | The quota banner carries the Manage plan button.                                                                                 | Review 2026-09-18: `QuotaBanners` had the `action` prop but `/app` never passed it; it does now.                                                                                                 | Dead prop found in review.                                                                                                                                                                                                                                                                                                                                                                                                   |
| 5     | (log rule)                                                                                                                       | Review 2026-09-18: the sweep-failure warning in `Auth.ts` annotates `causeToErrorMessage(cause)` instead of the raw `Cause`.                                                                     | A raw `Cause` is an unbounded structured value; the logging rule keeps those out of annotations.                                                                                                                                                                                                                                                                                                                             |
| 8     | Route-level test that the member-cap message reaches the response.                                                               | Review 2026-09-18: `e2e/members.spec.ts` seeds `MAX_ENTITLEMENTS.maxMembers` members and asserts the banner sentence on a refused add, headless in the regular `e2e` project.                    | No integration harness reaches an embedded server fn; building one for a single sentence was not worth it, and the E2E surface is the one a merchant sees.                                                                                                                                                                                                                                                                   |
| —     | (not planned) Plan switching end to end.                                                                                         | Review 2026-09-18: `e2e/plan.billing.spec.ts` in a new `billing` Playwright project, run only by `pnpm test:e2e:billing`, headed and by hand; `test:e2e` stays headless and never selects it.    | Shopify's pricing page sits behind a Cloudflare bot check that challenges headless browsers and cannot be solved in code. On the same-org dev store the page reads "Free to test" and the button "Test with this plan", not "Select".                                                                                                                                                                                        |

## 17. Verification record

§14's checklist, as actually run on 2026-09-18 against `sandbox-shop-01` on the **Basic** plan
(chosen over Pro so the 3-member cap was reachable). No `d1:reset` was needed — the local
Durable Object had already been recreated and carried the new DDL.

1. `pnpm typecheck`, `pnpm lint`, `pnpm test` (322 passing, 13 new), `pnpm graphql-codegen`,
   `pnpm fmt`: green.
2. **First sync**: `field=created_at`; `databaseSize` logged at 303,104 before and 425,984
   after; `ordersSeen=137 ordersUpserted=137 ordersInserted=137 lineItemsUpserted=218
ordersTruncated=0 sweptOrders=0 sweptRuns=0`.
3. **The open-work filter, on real data**: 147 stored orders, every one `UNFULFILLED`, none
   cancelled, none closed. The imported order numbers jump #1507 → #1558, which is the block of
   fulfilled or closed orders the filter excluded — the clearest evidence the deviation in §16
   row 1 behaves as intended.
4. **Second sync**: switched to `field=updated_at` and its window held nothing, so it ended at
   `onOrdersSyncEmpty`. The unfiltered branch therefore ran but was never handed a fulfilled
   order to re-import; only the unit test pins that.
5. **Quota counter**: `ordersThisMonth=15` against 147 imported — the `processedAt >= monthStart`
   exemption held, and the 30-day backfill did not burn the month's quota.
6. **Member cap**: adding an eleventh member to a 9-member shop on Basic was refused with "Your
   plan allows 3 members. Upgrade to add more.", and D1 confirmed no row was written.
7. **Auth sweep**: an expired `Verification` and `Session` seeded by hand were both gone after
   one magic-link request.
8. **Surfaces**: the over-quota banner renders identically on `/app` and the orders index; the
   admin shop page shows plan, orders-this-month, members, database size, last sweep and
   live-runs-limited.

Not exercised live, and covered only by integration tests with the constant lowered through the
seam in §16: the live-run ceiling (needs 5,000 live runs), a retention pass that actually
deletes (needs 90-day-old closed orders), and line-item truncation (needs a 251-item order).

The plans exist in the Partner Dashboard for `baton-local` and their copy matches `ENTITLEMENTS`
and the README table. `BILLING_ENABLED` is `"true"` in `wrangler.jsonc` — the project owner's
change, not the implementer's, and still to be decided per environment before shipping.
