...
- The bulk button does a different thing on the first click (open, unfulfilled orders created in the last 30 days) than on later clicks (anything updated since the last sync). §5 reworks it into one fixed-query import with polling progress and the agents SDK as the only run tracker.
...
Known failure: if polling times out, the Shopify bulk operation keeps running. A re-click submits a second operation, which Shopify rejects because one is already running, so the second click also fails. There is no cancel or adopt-existing path.
...
`bulkOperationRunQuery` hands Shopify a query and returns immediately with an operation id. Shopify runs it in the background and exposes `status` (`CREATED`, `RUNNING`, `COMPLETED`, `FAILED`, `CANCELED`), a running `objectCount`, and, when complete, a `url` to a JSONL file (7-day life). Two ways to learn it finished: poll `bulkOperation(id)` on a timer, or subscribe to the `bulk_operations/finish` webhook. Both are supported; neither is required. Polling is what motio does, and it is the better fit here because each poll also returns `objectCount`, which motio forwards as progress text ("Exported 120 records") through the agents SDK's `reportProgress` → `onWorkflowProgress` → socket (`../motio/src/lib/ScanWorkflow.ts:330-334`, `ShopAgent.ts:682`). A webhook gives no progress and adds a route. Drop the webhook idea.
...
1. **submit** (`step.do`): `bulkOperationRunQuery`. Return the operation id. No adopt logic: Baton's API version (2026-10) allows five concurrent bulk queries per shop, and the DO-side guard below already prevents a second run, so "already in progress" cannot occur from Baton's own clicks.
2. **poll** (loop): `step.sleep("5 seconds")` then `step.do` fetching `bulkOperation(id)`; on each pass `this.reportProgress({ objectCount })`. Exit on `COMPLETED` with `url`. On `FAILED`/`CANCELED`, or after **5 minutes** without completion, `bulkOperationCancel(id)` and fail with a message ("Shopify did not finish the export in time. Try again."). Five minutes is ten times the expected duration; it is when to stop the spinner, not when to expect success.
3. **stream** (`step.do`, default retries and timeout): the existing `onOrdersStream(url)`. Per-order transactions make a retry idempotent. Then `step.reportComplete`.

Error sink stays `Effect.onError` → `onOrdersSyncError` for the banner text.

**Is one already running?** Use the agents SDK's own tracking, not Shopify. `runWorkflow` records the instance in the DO's `cf_agents_workflows` table; `getWorkflows({ status: ["queued", "running"] })` is a local SQLite read inside the DO (`refs/agents/packages/agents/src/index.ts`, `getWorkflows`), and `onWorkflowComplete`/`onWorkflowError` flip the row. So `syncOrders` is: if `getWorkflows(...)` is non-empty, return "in flight"; else `runWorkflow`. Two admins clicking at once: the Durable Object runs one JavaScript thread, so the check is atomic with everything up to the first `await`. Set a plain instance field (`this.importStarting = true`) synchronously before `await runWorkflow(...)` and clear it after; the second click sees the flag and returns. No reservation row, no TTL, no existence probe. A wedged instance (row stuck on `running` because the Workflow died without reporting) is the one residual: a `getWorkflowStatus` refresh when the row is older than 10 minutes clears it, the same way motio's loader reaps a stuck scan.

Merchant sees: one button, "Import open orders", tooltip "Imports open, unfulfilled orders from the last 30 days. Safe to run again." While running: button disabled, spinner, progress line "Exporting… 120 orders" from `onWorkflowProgress`. On failure: critical banner with the message. On success: the orders list fills over the socket; optionally "Last imported 2 minutes ago".

| Component                  | Today                                                                                                                             | Rework                                                                                                                           |
| -------------------------- | --------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `OrdersSyncWorkflow.ts`    | 416 lines, `timedPollStep`, poll schedule, window params                                                                          | ~150 lines, three steps                                                                                                          |
| `ShopAgent.ts` sync sites  | ~57: `syncOrders`, `reserveSync`, `completeSync`, `failSync`, `clearSync`, `orderSyncWindow`, TTL, existence probe, storage guard | ~20: `syncOrders` (guard + `runWorkflow`), `onOrdersStream`, `onWorkflowProgress/Complete/Error`, `lastError`, `lastCompletedAt` |
...
| `ShopOrder.tags`           | stored and shown                                                                                                                  | removed                                                                                                                          |
...
- Cost of dropping the 90-day rule: closed orders stay for a year instead of three months. Storage is irrelevant (§ above). The only effect is the orders list, which must not show a year of fulfilled orders by default; if it does today, that is a list filter, not a purge rule.
...
- State the policy in the app: "Baton keeps closed orders for 90 days and open orders for a year. Shopify keeps everything." No merchant should read Baton as an archive.
- The 2 GB soft guard: confirmed, it gates only `syncOrders` (`ShopAgent.ts:1642`, the single call site). It goes away with the bulk sync (§5). If option C keeps a paginated import button, do not carry the guard over; the import is bounded by page count instead.

## 10. JSDoc drift and suspected bugs

Severity: H = billing or data loss, M = merchant-visible or operational, L = doc only.

| #   | Where                                                                                                  | JSDoc / label says                                   | Code does                                                                                                                                 | Sev | Change                                                                              |
| --- | ------------------------------------------------------------------------------------------------------ | ---------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | --- | ----------------------------------------------------------------------------------- |
| 1   | `OrdersSyncWorkflow.ts:311-347`                                                                        | —                                                    | After poll timeout the Shopify bulk op keeps running; re-click submits a new op, Shopify rejects it, another failure. No cancel or adopt. | M   | code: on "already in progress" poll the existing op, or `bulkOperationCancel` first |
| 2   | `app.orders.index.tsx:534`                                                                             | "Sync last 30 days"                                  | First run: open work only by `created_at`. Later: `updated_at` delta. Empty-state copy "No orders in the last 30 days" also wrong.        | M   | see §5                                                                              |
| 3   | `ShopAgent.ts:1995-2013`, `cycleAtOrderCeiling` JSDoc                                                  | —                                                    | Webhook at ceiling returns 2xx without storing; Shopify does not retry; order lost unless a later bulk window covers it. Undocumented.    | M   | doc on `cycleAtOrderCeiling`; consider auto delta-sync on rollover                  |
| 4   | `Domain.ts:99`, `README.md:67`                                                                         | `ordersPerCycle` must equal the meter's free tier    | Nothing verifies                                                                                                                          | M   | code: check on revalidation, warn on admin page                                     |
| 5   | Open-order retention                                                                                   | —                                                    | Never purged                                                                                                                              | M   | product decision, §9                                                                |
| 6   | `canStart` JSDoc `WorkflowRunRepository.ts:164`                                                        | manual attach skips line-item half                   | `setRun` also skips paid and cancelled/fulfilled gates; attach on a cancelled order succeeds                                              | M   | decide: document or add guard to `attachWorkflow`                                   |
| 7   | `hasLive` includes `done`                                                                              | Intended (`Domain.ts:2648`)                          | Quantity increase on a done line is silently lost                                                                                         | M   | doc on `reconcileOrder`, or flag done runs on increase                              |
| 8   | `orderSyncConstants.ts:12-14`                                                                          | overlap upserts are "free"                           | `>=` rewrites equal-`updatedAt` rows: line items replaced, reconcile re-run                                                               | L   | doc: "cheap, not free"; `>=` is required for line-item completion                   |
...
| 11  | `matchesLineItem` JSDoc `:198`, `OrderRepository.ts:525`                                               | `matchedWorkflowIds` written every pass              | Cancelled/fulfilled early-exit before write; `reconcileAll` skips unpaid, so badges can be stale                                          | L   | doc: scope to open orders; or clear on early exit                                   |
| 12  | `Domain.ts:545`                                                                                        | closed order untouched 90d is deleted                | also requires no pending/active run                                                                                                       | L   | doc                                                                                 |
| 13  | `ShopAgent.ts:2020-2024`                                                                               | quiet shop still ages out                            | no alarm; silent shop never sweeps                                                                                                        | L   | doc, or the alarm in §9                                                             |
| 14  | `ShopAgent.ts:1040-1045`                                                                               | 60 min TTL is "comfortably longer"                   | 10 min × 5 retries can exceed; the existence check is the real guard                                                                      | L   | doc                                                                                 |
...
| 17  | `OrderRepository.ts:1334`                                                                              | —                                                    | `lastFullSyncWindowStart` written, never read                                                                                             | L   | drop (falls out of §5)                                                              |
...
| 19  | `OrderRepository.ts:1411-1436`                                                                         | —                                                    | recount from `countedAt` and 90d sweep are safe only for cycles of a month or less; yearly is forbidden by README but unenforced          | L   | doc on `countedSince`                                                               |
| 20  | `OrderRepository.upsertOrder`                                                                          | —                                                    | The `updatedAt` guard's meaning (Shopify version check, not clock ordering; ties accepted on purpose) is not stated                       | L   | doc, per §3                                                                         |
| 21  | `docs/limits-and-plans-research.md:109`, `docs/limits-and-plans-plan.md:52`, `docs/rules-inventory.md` | stale sizes, overage "out of scope", stale line refs | —                                                                                                                                         | L   | delete or refresh                                                                   |

No correctness bug found in the upsert, meter, or tag-matching paths; each has rule-titled tests.
...
- **Timestamps**: keep the Shopify `updatedAt` version check; document it (§3).
- **Retention**: one rule, any order older than 365 days is deleted; no 90-day rule; no alarm (§9).
...
- **Manual attach**: refuse on cancelled and fulfilled orders; still allowed on unpaid. One guard in `attachWorkflow`, one rule-titled test.

Recommended, awaiting a call:

1. **Bulk sync (§5)**: build the rework, a single fixed-query import with polling progress and the agents SDK as the only run tracker; retire the current implementation; no admin-link import.
2. **Quantity increase on a done line (#7)**: flag the done run `quantity_changed`, the same flag active runs get today, so it surfaces on the order page and run list. No new run, automatic or manual. The merchant already has the deliberate action you described: **Undo** reopens a finished step (`Domain.ts:2608`, `undoBlockedBy`), and undoing the last step takes a `done` run back to active (`Domain.ts:2643`). Once reopened, the run is active and the existing `quantity_changed` handling applies. So the whole change is: extend the flagging in `adjust` to include `done` runs. No index change, no "start another run" path.
