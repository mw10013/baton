...
/**
 * Shared workflow detail editor, rendered by both `/app/workflows/$workflowId`
 * (item scope) and `/app/order-workflow/$workflowId` (order scope). The read
 * is a loader on the owning route, every write is one socket RPC, and
 * `router.invalidate()` re-reads after each. Every write returns a tagged
 * result that is copy-mapped here rather than thrown, so a taken name lands
 * on the field and a limit lands in the banner.
 *
 ...
 * touch the draft.
 *
 * `backHref`/`backLabel` are the breadcrumb back to the owning list, and
 * `deletedRedirectTo` is where a successful delete navigates (the detail page
 * is gone, so it cannot stay).
 */
export function WorkflowDetailPage({
  ...
  readonly detail: Domain.WorkflowDetailView | null;
  readonly backHref: string;
  readonly backLabel: string;
  readonly deletedRedirectTo: string;
}) {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: async (result) => {
      if (result._tag === "Deleted") {
        await router.navigate({ to: deletedRedirectTo });
        return;
      }
