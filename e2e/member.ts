 ...
 * the embedded wrapper uses. It is load-bearing for the same reason
 * `data-app-hydrated` is: until React attaches its listeners the SSR'd markup
 * is fully painted and fully dead, so a click passes every Playwright
 * actionability check and the activation is swallowed — a silent drop, no
 * error, surfacing much later as an assertion for a result the click never
 * requested.
 *
